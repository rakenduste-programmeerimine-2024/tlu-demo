...
export default function SearchBar() {
    ...
    const handleSearch = (l: ChangeEvent<HTMLInputElement>) => {
        if(l.target.value == ""){
            setSearch([])
            return false
        }
        setSearch(results.filter(entry => entry.title.toLowerCase().includes(l.target.value.toLowerCase())).slice(0,8));
    };
